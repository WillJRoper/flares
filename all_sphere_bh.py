...
def spherical_region(sim, snap):
    ...
    bh_cood = E.read_array('PARTDATA', sim, snap, '/PartType1/Coordinates',
                           noH=True, physicalUnits=False, numThreads=4)  # bh particle coordinates

    hull = ConvexHull(bh_cood)

    cen = [np.median(bh_cood[:, 0]), np.median(bh_cood[:, 1]), np.median(bh_cood[:,2])]
    pedge = bh_cood[hull.vertices]  #edge particles
...
def blend(rgb_DM, rgb_gas):
    ...
    bhgas_output = blend1.Overlay()
    rgb_output = bhgas_output
...
def spheregrid(snap, num):
    ...
    for reg, (i, j) in zip(regions, pltgrid):
        ...
        hdf = h5py.File(f'spheresdata/spheregird_img_data_{reg}_{snap}_bh.hdf5', 'r')
    ...
    fig.savefig('plots/spheres/BH/bh_grid_sphere_snap' + snap + '_angle%05d.png'%num,
...
# bh_scene = set_up_single_sphere(reg, snap, csoft, centre, part_type=5)
#
# # Load the current snapshot data
# hdf = h5py.File(f'spheresdata/spheregird_img_data_{reg}_{snap}_bh.hdf5', 'w')
#
# for num, p in enumerate(ps):
#
#     rgb_bh, extent = get_single_sphere_imgs(bh_scene, p, t=0)
#
#     frame = hdf.create_group(str(num))
#     frame.create_dataset('img', shape=rgb_bh.shape, dtype=float, data=rgb_bh,
#                             compression='gzip')
