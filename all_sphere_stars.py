...
def spherical_region(sim, snap):
    ...
    stars_cood = E.read_array('PARTDATA', sim, snap, '/PartType1/Coordinates',
                           noH=True, physicalUnits=False, numThreads=4)  # stars particle coordinates

    hull = ConvexHull(stars_cood)

    cen = [np.median(stars_cood[:, 0]), np.median(stars_cood[:, 1]), np.median(stars_cood[:,2])]
    pedge = stars_cood[hull.vertices]  #edge particles
...
def blend(rgb_DM, rgb_gas):
    ...
    starsgas_output = blend1.Overlay()
    rgb_output = starsgas_output
...
def spheregrid(snap, num):
    ...
    for reg, (i, j) in zip(regions, pltgrid):
        ...
        hdf = h5py.File(f'spheresdata/spheregird_img_data_{reg}_{snap}_stars.hdf5', 'r')
        ...
        rgb = cmap(get_normalised_image(scaled_img, vmin=5, vmax=16.5))
    ...
    fig.savefig('plots/spheres/Stars/stars_grid_sphere_snap' + snap + '_angle%05d.png'%num,
...
# stars_scene = set_up_single_sphere(reg, snap, csoft, centre, part_type=4)
#
# # Load the current snapshot data
# hdf = h5py.File(f'spheresdata/spheregird_img_data_{reg}_{snap}_stars.hdf5', 'w')
#
# for num, p in enumerate(ps):
#
#     rgb_stars, extent = get_single_sphere_imgs(stars_scene, p, t=0)
#
#     frame = hdf.create_group(str(num))
#     frame.create_dataset('img', shape=rgb_stars.shape, dtype=float, data=rgb_stars,
#                             compression='gzip')
