    ...
    for ind, i in enumerate(gals):
        ...
        if len(progs) == 0:
            ...
            if len(feedback[part_halo_ids == i]) > 0:
                delta_fbs[ind] = np.max(feedback[part_halo_ids == i])
            else:
                delta_fbs[ind] = 0
...
def main_change(masslim=1e8, hmrcut=False, load=False):
    ...
    if load:
        ...
        with open('changeinsize_feedbackmax.pck', 'rb') as pfile1:
        ...
        with open('changeinsize_feedbackmax.pck', 'wb') as pfile1:
    ...
    for ax, snap, (i, j) in zip([ax1, ax2, ax3, ax4, ax5, ax6, ax7, ax8, ax9], snaps,
        ...
        if j == 0:
            ax.set_ylabel('$\mathrm{max}(f_{\mathrm{th}})$')
    ...
    fig.savefig('plots/change_in_massvsfeedbackmax.png', bbox_inches='tight')
    ...
    for ax, snap, (i, j) in zip([ax1, ax2, ax3, ax4, ax5, ax6, ax7, ax8, ax9], snaps,
        ...
        if len(xs_plt) > 0:
            ...
            # Label colorbars
            cbar1.ax.set_xlabel(r'$\mathrm{max}(f_{\mathrm{th}})$', labelpad=1.5, fontsize=9)
    ...
    fig.savefig('plots/change_in_halfmassradius_feedbackmax.png', bbox_inches='tight')
    ...
    for ax, snap, (i, j) in zip([ax1, ax2, ax3, ax4, ax5, ax6, ax7, ax8, ax9], snaps,
        ...
        # Label axes
        if i == 2:
            ax.set_xlabel(r'$\mathrm{max}(f_{\mathrm{th}})$')
        if j == 0:
            ax.set_ylabel('$R_{1/2,\mathrm{\star}}/R_{1/2,\mathrm{\star},\mathrm{main prog}}$')
    ...
    fig.savefig('plots/change_in_halfmassradiusvsfeedbackmax.png', bbox_inches='tight')
