...
def main_change(masslim=1e8, hmrcut=False, load=False):
    ...
    for ax, snap, (i, j) in zip([ax1, ax2, ax3, ax4, ax5, ax6, ax7, ax8, ax9], snaps,
        ...
        if j == 0:
            ax.set_ylabel('$\mathrm{min}(f_{\mathrm{th}})$')
    ...
    fig.savefig('plots/change_in_massvsfeedbackmin.png', bbox_inches='tight')
    ...
    for ax, snap, (i, j) in zip([ax1, ax2, ax3, ax4, ax5, ax6, ax7, ax8, ax9], snaps,
        ...
        if len(xs_plt) > 0:
            ...
            # Label colorbars
            cbar1.ax.set_xlabel(r'$\mathrm{min}(f_{\mathrm{th}})$', labelpad=1.5, fontsize=9)
    ...
    fig.savefig('plots/change_in_halfmassradius_feedbackmin.png', bbox_inches='tight')
    ...
    for ax, snap, (i, j) in zip([ax1, ax2, ax3, ax4, ax5, ax6, ax7, ax8, ax9], snaps,
        ...
        # Label axes
        if i == 2:
            ax.set_xlabel(r'$\mathrm{min}(f_{\mathrm{th}})$')
        if j == 0:
            ax.set_ylabel('$R_{1/2,\mathrm{\star}}/R_{1/2,\mathrm{\star},\mathrm{main prog}}$')
