...
def partDirectProgDesc(snap, prog_snap, desc_snap, path):
    ...
    part_ids = np.concatenate([gas_snap_part_ids, dm_snap_part_ids, star_snap_part_ids, bh_snap_part_ids])
    part_types = np.concatenate([gas_part_types, dm_part_types, star_part_types, bh_part_types])
    ...
    if part_type == 1:
        ...
        try:
            desc_part_ids = E.read_array('PARTDATA', path, desc_snap, 'PartType' + str(part_type) + '/ParticleIDs',
                                         numThreads=8)
            desc_masses = np.full_like(desc_part_ids, dm_mass)
        except ValueError:
            desc_masses = np.array([])
            desc_part_ids = np.array([])
    else:
        try:
            snap_masses = E.read_array('PARTDATA', path, snap, 'PartType' + str(part_type) + '/Mass',
                                       numThreads=8) / 0.6777
        except ValueError:
            snap_masses = np.array([])
        try:
            prog_masses = E.read_array('PARTDATA', path, prog_snap, 'PartType' + str(part_type) + '/Mass',
                                       numThreads=8) / 0.6777
            prog_part_ids = E.read_array('PARTDATA', path, prog_snap, 'PartType' + str(part_type) + '/ParticleIDs',
                                         numThreads=8)
        except ValueError:
            ...
            prog_part_ids = np.array([])
        try:
            desc_masses = E.read_array('PARTDATA', path, desc_snap, 'PartType' + str(part_type) + '/Mass',
                                       numThreads=8) / 0.6777
            desc_part_ids = E.read_array('PARTDATA', path, desc_snap, 'PartType' + str(part_type) + '/ParticleIDs',
                                         numThreads=8)
        except ValueError:
...
def mainDirectProgDesc(snap, prog_snap, desc_snap, path, savepath='MergerGraphs/', part_types=(0, 1, 4, 5)):
    ...
    if 1 in part_types:
        ...
        dm_result_tup = part_type_contribution(path, snap, prog_snap, desc_snap, 1, nprogs, ndescs, prog_start_index,
                                                 desc_start_index, sim_haloids, progs, descs, index_haloids)
        prog_dm_mass_conts, desc_dm_mass_conts, prog_dm_masses, desc_dm_masses, masses = dm_result_tup

        # Create file to store this snapshots graph results
        hdf = h5py.File(savepath + 'SubMgraph_' + snap + '.hdf5', 'r+')

        hdf.create_dataset('prog_dm_mass_contribution', shape=prog_dm_mass_conts.shape, dtype=float,
                           data=prog_dm_mass_conts, compression='gzip')
        hdf.create_dataset('desc_dm_mass_contribution', shape=desc_dm_mass_conts.shape, dtype=float,
                           data=desc_dm_mass_conts, compression='gzip')
        hdf.create_dataset('prog_dm_masses', shape=prog_dm_masses.shape, dtype=float,
