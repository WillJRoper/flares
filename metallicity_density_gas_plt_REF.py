...
def get_data(load=False):
    ...
    if load:
        ...
        with open('metvsd_REF.pck', 'rb') as pfile1:
        ...
        for snap in snaps:
            ...
            gas_bd_dict[snap] = gas_den
            gas_met_dict[snap] = gas_met

        with open('metvsd_REF.pck', 'wb') as pfile1:
...
gas_met = np.concatenate(list(gas_met_dict.values()))
gas_bd = np.concatenate(list(gas_bd_dict.values())) * 10**10
...
for ax, snap, (i, j) in zip([ax1, ax2, ax3, ax4, ax5, ax6, ax7, ax8, ax9], snaps,
    ...
    metal_mass_fractions = np.array(gas_met_dict[snap])
    gas_bd = (np.array(gas_bd_dict[snap]) * 10**10 * Msun / Mpc ** 3 / mh).to(1 / cm ** 3).value
