...
for snap, color, ax, (i, j) in zip(snaps, matplotlib.cm.get_cmap('plasma')(np.linspace(0, len(snaps))),
    ...
    M_11p5 = M_11p5[np.where(M_11p5 != 0.0)]
    M_9p5 = M_9p5[np.where(M_9p5 != 0.0)]
    M_7p5 = M_7p5[np.where(M_7p5 != 0.0)]

    bins = np.arange(-4.0, 2.5, 0.4)
    ...
    # Plot each histogram
    ax.plot(bin_cents, np.log10(H_11p5 / interval / (3.2 * 10 ** 3) ** 3), color=color,
            label='$z_{\mathrm{reion}}=11.5$')
    ax.plot(bin_cents, np.log10(H_9p5 / interval / (3.2 * 10 ** 3) ** 3), color=color, linestyle='--',
            label='$z_{\mathrm{reion}}=9.5$')
    ax.plot(bin_cents, np.log10(H_7p5 / interval / (3.2 * 10 ** 3) ** 3), color=color, linestyle=':',
            label='$z_{\mathrm{reion}}=7.5$')
    ...
    # Label axes
    if i == 1:
        ax.set_xlabel(r'$\log_{10}(SFR/[M_\odot/\mathrm{yr}])$')
    if j == 0:
        ax.set_ylabel(r'$\log_{10}(SFR \mathrm{Mpc}^{-3} \mathrm{dex}^{-1})$')
...
# Save figure
fig.savefig('SFR_reion_comp.png', bbox_inches='tight', dpi=300)
